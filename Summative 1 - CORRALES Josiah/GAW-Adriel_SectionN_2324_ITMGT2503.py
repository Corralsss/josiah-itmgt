...
final_script = "" 
jsondata_values = jsondata.values()
list_script = list(jsondata.values())
unnecessary_punctuation = r"&$@[].,'#()-\"!?’_;:/"
pieces_of_text = ['<P>', '</P>', '<B>', '</B>', '<I>', '</I>']

for i in range(len(list_script)): 
    final_script += list_script[i]

final_script = str(final_script)
final_script = final_script.upper()

for i in pieces_of_text: 
    final_script = final_script.replace(i,'')

for i in range(len(unnecessary_punctuation)): 
    final_script = final_script.replace(unnecessary_punctuation[i],' ')
...
for i in words:
    if len(i) >= 3: 
        wordcount_dictionary[i] = words.count(i)
...
list_keys = list(wordcount_dictionary.keys())
list_values = list(wordcount_dictionary.values())
list = []

for i in range(len(wordcount_dictionary)): 
    list.append((str(list_keys[i])))
    list.append((str(list_values[i])))
    tuples = tuple(list) 
    final_wordlist_of_tuples.append(tuples)
    list = []

final_wordlist_of_tuples.sort(key=lambda x: x[1], reverse=False)
...
while lumber != 12000:
    if lumber == 12000 and gold == 18000: 
        cleric -= 1 
        gold -= 750
        lumber -= 300
        food += 75
...
def Collatz_winner(number_list):
    ...
    while i != len(number_list): 
        collatz_sequence = [number_list[i]]
        j = number_list[int(i)]
        while j != 1: 
            if j % 2 == 0: 
                j = j / 2
                collatz_length += 1
                collatz_sequence.append(int(j))
            elif j % 2 != 0: 
                j = (j*3) + 1
                collatz_length += 1
                collatz_sequence.append(int(j))
    ...
    for i in range(len(list_max_collatz)):
        if list_max_collatz[i] == max(list_max_collatz) and list_collatz_length[i] == max(list_collatz_length): 
            return int(number_list[i])
